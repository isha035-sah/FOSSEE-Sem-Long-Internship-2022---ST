package com.example.TravlellerBlog.controller;
...
import com.example.TravlellerBlog.service.ImageGalleryService;
...
public class ImageGalleryController {
    ...
    String showProductDetails(@RequestParam("password") Long password, Optional<ImageGallery> imageGallery, Model model) {
        try {
            log.info("Id :: " + password);
            if (password != 0) {
                imageGallery = imageGalleryService.getImageById(password);

                log.info("products :: " + imageGallery);
                if (imageGallery.isPresent()) {
                    model.addAttribute("id", imageGallery.get().getPassword());
